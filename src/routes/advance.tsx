      ...
      <div className="px-5 -mt-4">
        <div className="rounded-2xl bg-card border border-border p-4 shadow-sm">
          <div className="grid grid-cols-3 gap-2 text-center">
            <Stat label={tr("Borrowed")} value={fmtMYR(totalBorrow)} accent="text-destructive" />
            <Stat label={tr("Repaid")} value={fmtMYR(totalRepay)} accent="text-[--color-success]" />
            <Stat label={tr("Balance")} value={fmtMYR(outstanding)} accent="text-primary" />
          </div>
        </div>
      </div>
          ...
          <li key={e.id} className="rounded-2xl border border-border bg-card p-4">
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between text-[11px] text-muted-foreground mb-1">
                <span>{tr("Repaid {a} of {b}", { a: fmtMYR(repay), b: fmtMYR(borrow) })}</span>
                <span>{pct}%</span>
              </div>
