...
function ReviewForm({
  ...
  showReassessNotice: boolean;
  onRunReassess: () => void;
  onKeepAssessment: () => void;
}) {
  const { tr } = useT();
  const photoUrls = useMemo(
    () => photos.map((p) => URL.createObjectURL(p.file)),
    [photos],
  );
  useEffect(
        ...
        {showReassessNotice && (
          <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 space-y-2">
            <p className="text-xs font-medium text-amber-700 dark:text-amber-300">
              {tr("New photos were added after the last assessment.")}
            </p>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={onRunReassess}
                className="flex-1 inline-flex items-center justify-center gap-1 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground"
              >
                <Sparkles className="h-3.5 w-3.5" /> {tr("Run AI Re-assessment")}
              </button>
              <button
                type="button"
                onClick={onKeepAssessment}
                className="flex-1 rounded-lg border border-border bg-background px-3 py-2 text-xs font-semibold"
              >
                {tr("Keep Existing Assessment")}
              </button>
            </div>
          </div>
        )}
